...
% 20150718, hiroshi: test training with original clean image/global clean image 
Image1 = varargin{1};
Image2 = varargin{2};
...
if(prod([NN1,NN2]-bb+1)> maxNumBlocksToTrainOn)
    ...
    for i = 1:maxNumBlocksToTrainOn
        ...
        currBlock = Image1(row:row+bb-1,col:col+bb-1);
        blkMatrix(:,i) = currBlock(:);
    end
else
    blkMatrix = im2col(Image1,[bb,bb],'sliding');
end
...
%20150720, hiroshi: test checkerboard
[blocks_checker,idx2] = my_im2col(varargin{3},[bb,bb],slidingDis);
...
%     Coefs = OMPerr(Dictionary,blocks(:,jj:jumpSize),errT);
    %20150720, hiroshi: test checkerboard
    Coefs = OMPerr_checkerboard(Dictionary,blocks(:,jj:jumpSize),errT, blocks_checker);
