...
[shrinkImg dotImg interpImg checkerImg] = imageSampling(IMin0, 2, 'cross');
IMin = dotImg;
...
% IMin = imnoise(IMin/255, 'salt & pepper')*255;
PSNRIn = 20*log10(255/sqrt(mean((IMin(:)-IMin0(:)).^2)));

% % 20150718, hiroshi: temp test for median filter comparison
% J = medfilt2(IMin);
% figure
% subplot(1,3,1), imshow(IMin0, []);
% subplot(1,3,2), imshow(IMin, []);
% subplot(1,3,3), imshow(J, []);

% 20150718, hiroshi: assistant image
imageName1 = 'boat.png';
[IMin1,pp]=imread(strcat([pathForImages,imageName1]));
IMin1=im2double(IMin1);
if (length(size(IMin1))>2)
    IMin1 = rgb2gray(IMin1);
end
if (max(IMin1(:))<2)
    IMin1 = IMin1*255;
end
...
% % 20150720, hiroshi: checkerboard test
% % [IoutDCT,output] = denoiseImageDCT(IMin, sigma, K, checkerImg);
% 
...
% 20150720, hiroshi: checkerboard test
[IoutGlobal,output] = denoiseImageGlobal(IMin, sigma,K, checkerImg);
...
%==========================================================================
% % [IoutAdaptive,output] = denoiseImageKSVD(IMin, sigma, K, IMin0, IMin1);
% % 20150720, hiroshi: checkerboard test
% [IoutAdaptive,output] = denoiseImageKSVD(IMin, sigma, K, IMin0, IMin1, checkerImg);
% 
